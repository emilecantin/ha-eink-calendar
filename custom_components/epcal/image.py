...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            EpcalBitmapImage(coordinator, entry, "black_top", IMAGE_BLACK_TOP_NAME),
            EpcalBitmapImage(
                coordinator, entry, "black_bottom", IMAGE_BLACK_BOTTOM_NAME
            ),
            EpcalBitmapImage(coordinator, entry, "red_top", IMAGE_RED_TOP_NAME),
            EpcalBitmapImage(coordinator, entry, "red_bottom", IMAGE_RED_BOTTOM_NAME),
        ]
    )


class EpcalBitmapImage(ImageEntity):
    """Image entity serving bitmap layers for ESP32."""

    def __init__(
        self,
        coordinator: EpcalDataCoordinator,
        entry: ConfigEntry,
        layer_type: str,
        entity_name: str,
    ) -> None:
        ...
        self._attr_name = f"{entry.data.get(CONF_DEVICE_NAME, 'EPCAL')} {entity_name.replace('_', ' ').title()}"
