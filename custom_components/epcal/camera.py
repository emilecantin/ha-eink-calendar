...
from .const import CAMERA_NAME, CONF_DEVICE_NAME, DOMAIN
...
async def async_setup_entry(
    ...
    async_add_entities([EpcalPreviewCamera(coordinator, entry)])


class EpcalPreviewCamera(Camera):
    ...
    def __init__(self, coordinator: EpcalDataCoordinator, entry: ConfigEntry) -> None:
        ...
        self.entry = entry
        self._attr_name = (
            f"{entry.data.get(CONF_DEVICE_NAME, 'EPCAL')} {CAMERA_NAME.title()}"
        )
